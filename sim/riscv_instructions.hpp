...
public:
    ...
    // U-Type immediate
    int32_t getImmU() const {
        return raw & 0xFFFFF000;
    }
    
    // J-Type immediate
    int32_t getImmJ() const {
        uint32_t imm = ((raw >> 31) << 20) |
                       (((raw >> 12) & 0xFF) << 12) |
                       (((raw >> 20) & 0x1) << 11) |
                       (((raw >> 21) & 0x3FF) << 1);
    ...
    static std::string toAssembly(const DecodedInstruction& decoded) {
        ...
        if (decoded.format == "R") {
            asm_str += " x" + std::to_string(decoded.rd) + 
                      ...
                      ", x" + std::to_string(decoded.rs2);
        } else if (decoded.format == "I") {
            if (decoded.opcode == Opcode::OP_LD || decoded.opcode == Opcode::OP_JALR) {
                asm_str += " x" + std::to_string(decoded.rd) + 
            ...
            asm_str += " x" + std::to_string(decoded.rs1) + 
                      ", x" + std::to_string(decoded.rs2) + 
                      ", " + std::to_string(decoded.imm);
...
private:
    ...
    static std::string decodeRType(const DecodedInstruction& decoded) {
        switch (static_cast<Funct3ALU>(decoded.funct3)) {
            ...
            case Funct3ALU::SLL: return "sll";
            case Funct3ALU::SLT: return "slt";
            case Funct3ALU::SLTU: return "sltu";
            case Funct3ALU::XOR: return "xor";
            case Funct3ALU::SRL_SRA:
                return (decoded.funct7 == 0x00) ? "srl" : "sra";
            case Funct3ALU::OR: return "or";
            case Funct3ALU::AND: return "and";
    ...
    static std::string decodeIType(const DecodedInstruction& decoded) {
        if (decoded.opcode == Opcode::OP_LD) {
            switch (static_cast<Funct3LDST>(decoded.funct3)) {
                case Funct3LDST::BYTE: return "lb";
                case Funct3LDST::HALF: return "lh";
                case Funct3LDST::WORD: return "lw";
                case Funct3LDST::BYTE_U: return "lbu";
                case Funct3LDST::HALF_U: return "lhu";
            ...
            switch (static_cast<Funct3ALU>(decoded.funct3)) {
                case Funct3ALU::ADD_SUB: return "addi";
                case Funct3ALU::SLL: return "slli";
                case Funct3ALU::SLT: return "slti";
                case Funct3ALU::SLTU: return "sltiu";
                case Funct3ALU::XOR: return "xori";
                case Funct3ALU::SRL_SRA:
                    return (decoded.funct7 == 0x00) ? "srli" : "srai";
                case Funct3ALU::OR: return "ori";
                case Funct3ALU::AND: return "andi";
    ...
    static std::string decodeSType(const DecodedInstruction& decoded) {
        switch (static_cast<Funct3LDST>(decoded.funct3)) {
            case Funct3LDST::BYTE: return "sb";
            case Funct3LDST::HALF: return "sh";
            case Funct3LDST::WORD: return "sw";
    ...
    static std::string decodeBType(const DecodedInstruction& decoded) {
        switch (static_cast<Funct3BR>(decoded.funct3)) {
            case Funct3BR::BEQ: return "beq";
            case Funct3BR::BNE: return "bne";
            case Funct3BR::BLT: return "blt";
            case Funct3BR::BGE: return "bge";
            case Funct3BR::BLTU: return "bltu";
            case Funct3BR::BGEU: return "bgeu";
    ...
    static std::string decodeJType(const DecodedInstruction&) {
        return "jal";
    }
};
...
public:
    ...
    static uint32_t ADD(uint8_t rd, uint8_t rs1, uint8_t rs2) {
        return encodeRType(0x00, rs2, rs1, 0x0, rd, static_cast<uint8_t>(Opcode::OP_ALU_R));
    }
    
    static uint32_t SUB(uint8_t rd, uint8_t rs1, uint8_t rs2) {
        return encodeRType(0x20, rs2, rs1, 0x0, rd, static_cast<uint8_t>(Opcode::OP_ALU_R));
    }
    
    static uint32_t AND(uint8_t rd, uint8_t rs1, uint8_t rs2) {
        return encodeRType(0x00, rs2, rs1, 0x7, rd, static_cast<uint8_t>(Opcode::OP_ALU_R));
    }
    
    static uint32_t OR(uint8_t rd, uint8_t rs1, uint8_t rs2) {
        return encodeRType(0x00, rs2, rs1, 0x6, rd, static_cast<uint8_t>(Opcode::OP_ALU_R));
    }
    
    static uint32_t XOR(uint8_t rd, uint8_t rs1, uint8_t rs2) {
        return encodeRType(0x00, rs2, rs1, 0x4, rd, static_cast<uint8_t>(Opcode::OP_ALU_R));
    }
    
    static uint32_t SLL(uint8_t rd, uint8_t rs1, uint8_t rs2) {
        return encodeRType(0x00, rs2, rs1, 0x1, rd, static_cast<uint8_t>(Opcode::OP_ALU_R));
    }
    
    static uint32_t SRL(uint8_t rd, uint8_t rs1, uint8_t rs2) {
        return encodeRType(0x00, rs2, rs1, 0x5, rd, static_cast<uint8_t>(Opcode::OP_ALU_R));
    }
    
    static uint32_t SRA(uint8_t rd, uint8_t rs1, uint8_t rs2) {
        return encodeRType(0x20, rs2, rs1, 0x5, rd, static_cast<uint8_t>(Opcode::OP_ALU_R));
    }
    
    static uint32_t SLT(uint8_t rd, uint8_t rs1, uint8_t rs2) {
        return encodeRType(0x00, rs2, rs1, 0x2, rd, static_cast<uint8_t>(Opcode::OP_ALU_R));
    }
    
    static uint32_t SLTU(uint8_t rd, uint8_t rs1, uint8_t rs2) {
        return encodeRType(0x00, rs2, rs1, 0x3, rd, static_cast<uint8_t>(Opcode::OP_ALU_R));
    }
    
    static uint32_t ADDI(uint8_t rd, uint8_t rs1, int16_t imm) {
        return encodeIType(imm, rs1, 0x0, rd, static_cast<uint8_t>(Opcode::OP_ALU_I));
    }
    
    static uint32_t ANDI(uint8_t rd, uint8_t rs1, int16_t imm) {
        return encodeIType(imm, rs1, 0x7, rd, static_cast<uint8_t>(Opcode::OP_ALU_I));
    }
    
    static uint32_t ORI(uint8_t rd, uint8_t rs1, int16_t imm) {
        return encodeIType(imm, rs1, 0x6, rd, static_cast<uint8_t>(Opcode::OP_ALU_I));
    }
    
    static uint32_t XORI(uint8_t rd, uint8_t rs1, int16_t imm) {
        return encodeIType(imm, rs1, 0x4, rd, static_cast<uint8_t>(Opcode::OP_ALU_I));
    }
    
    static uint32_t SLLI(uint8_t rd, uint8_t rs1, uint8_t shamt) {
        return encodeIType(shamt & 0x1F, rs1, 0x1, rd, static_cast<uint8_t>(Opcode::OP_ALU_I));
    }
    
    static uint32_t SRLI(uint8_t rd, uint8_t rs1, uint8_t shamt) {
        return encodeIType(shamt & 0x1F, rs1, 0x5, rd, static_cast<uint8_t>(Opcode::OP_ALU_I));
    }
    
    static uint32_t SRAI(uint8_t rd, uint8_t rs1, uint8_t shamt) {
        return encodeIType(0x400 | (shamt & 0x1F), rs1, 0x5, rd, static_cast<uint8_t>(Opcode::OP_ALU_I));
    }
    
    static uint32_t SLTI(uint8_t rd, uint8_t rs1, int16_t imm) {
        return encodeIType(imm, rs1, 0x2, rd, static_cast<uint8_t>(Opcode::OP_ALU_I));
    }
    
    static uint32_t SLTIU(uint8_t rd, uint8_t rs1, int16_t imm) {
        return encodeIType(imm, rs1, 0x3, rd, static_cast<uint8_t>(Opcode::OP_ALU_I));
    }
    ...
    static uint32_t JALR(uint8_t rd, uint8_t rs1, int16_t imm) {
        return encodeIType(imm, rs1, 0x0, rd, static_cast<uint8_t>(Opcode::OP_JALR));
    }
