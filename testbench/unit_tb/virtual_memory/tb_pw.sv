...
module tb_page_walker;
  ...
  task automatic wait_resp();
    ...
    while (!pw_if.ready) begin
      ...
      if (timeout_cnt > 200) begin
        $display("[PW DEBUG] timeout! busy=%b, ready=%b, state=%s", 
                 pw_if.busy, pw_if.ready, dut.pw_state_q.name());
  ...
  task automatic check_ok(input ppn_t exp_ppn, input string test_name);
    if (pw_if.page_fault) begin
      $fatal(1, "[PW] %s: esperado OK, got page_fault (cause=%s)", 
             test_name, pw_if.fault_cause.name());
    end
  ...
  task automatic check_fault(input page_fault_t exp_cause, input string test_name);
    ...
    if (pw_if.fault_cause !== exp_cause) begin
      $fatal(1, "[PW] %s: fault cause mismatch exp=%s got=%s", 
             test_name, exp_cause.name(), pw_if.fault_cause.name());
    end
